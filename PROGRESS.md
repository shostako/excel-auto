...
## 次セッションへの引き継ぎ
- **加工NW関連マクロ整備完了**: 以下すべてに加工NW統合済み
  - 見せる表_加工: ショット数をNWから取得
  - 一括_加工: NWシート同期・マクロ実行追加
  - クエリ参照元変更: 日報加工クエリの複数月結合追加
- **ロット数量テーブル廃止準備完了**: 9マクロすべてがロット数量テーブル不参照に
  - 手直し系: 成形T, 塗装T, 加工T
  - 廃棄系: 成形H, 塗装H, 加工H
  - 流出系: 成形G, 塗装G, 加工G
- **加工NW（日報ワースト順）マクロ完成**: 複雑な集計ロジック実装済み
  - パターンA（62-xxxxx）+ パターンB（単+特定数字）+ 補給品
- **手直しクエリ**: 年単位ファイル単独参照（月跨ぎ結合なし）
...
- **参照すべきリソース**:
  ...
  - `src/m転記_見せる表_加工.bas`（NWからショット数取得）
  - `src/mクエリ参照元変更_複数月対応.bas`（日報加工クエリ追加版）
