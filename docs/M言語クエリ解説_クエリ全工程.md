# M言語クエリ解説: クエリ全工程（業務背景説明付き）

## 1. このクエリの目的・概要

このクエリは、製造業の**全工程における作業者別生産実績データ**を処理・集計するためのPower Query（M言語）です。前回解説した「クエリ成形」が機械・号機の稼働状況を分析するのに対し、この「クエリ全工程」は**作業者の作業状況**を分析することに重点を置いています。
...
データソースは「生産実績データ」のExcelファイル（例：2506.xlsx）で、これを読み込んで分析しやすい形に整形します。
...
```
- ネットワーク上の共有フォルダからExcelファイルを読み込み
...
このクエリを実行すると、以下の列を持つ集計テーブルが生成されます：
...
クエリ成形が「設備をどう動かすか」を分析するのに対し、クエリ全工程は「人をどう配置するか」を分析するためのデータを提供します。
