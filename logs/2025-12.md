...
### フィルターマクロ オートフィルター方式への全面変更

**背景**:
- 既存の行非表示方式（`Row.Hidden = True/False`）では複合フィルターが複雑化
- 「全品番」選択時にその列だけ解除することが困難
- 手動オートフィルターとの共存が不可能

**解決策**: Excelのオートフィルター機能を直接操作する方式に変更

**変更内容**:
```vba
' Before: 行の直接操作
tbl.DataBodyRange.EntireRow.Hidden = False  ' リセット
ws.Rows(rowNum).Hidden = True               ' フィルター適用
...
**「稼働日」「合計」常時表示**:
- 各フィルター対象列自体に「稼働日」「合計」を追加してフィルター
- 例: 製品名列で「製品A」選択時 → `Array("製品A", "稼働日", "合計")`でフィルター

**対象ファイル**:
| ファイル | フィルター条件 |
|---------|--------------|
| m完成品フィルター.bas | B3の値, 稼働日, 合計 |
| m側板フィルター.bas | C3の値, 稼働日, 合計 |
| m小部品フィルター.bas | D3の値, 稼働日, 合計 |
| m項目フィルター.bas | E3の値, 稼働日, 合計 |
...
**コード量**: 大幅削減（オートフィルター操作はシンプル）

### 項目追加マクロ 全テーブル対応（汎用化）

**背景**:
- 既存の項目追加マクロは`_完成品`系テーブル専用だった
- `_core`、`_slitter`、`_acf`系テーブルにも対応が必要

**テーブル別列構成**:
| 列名 | _完成品 | _core/_slitter/_acf |
|-----|---------|---------------------|
| 製品名 | ○ | × |
| 側板 | ○ | × |
| 詳細 | × | ○ |
...
If colProduct > 0 Then
    ws.Cells(srcRow, ...).Copy
    ws.Cells(srcRow + 1, ...).PasteSpecial xlPasteAll
End If
```

**項目削除マクロ**: 変更なし（「項目」列のみ使用のため全テーブルで動作可能）

### settings.local.json PermissionRequestフック削除

**背景**:
- Delegateモードはplanモードと併用不可
- planモードで計画立案 → Delegateで実行、という切り替えがセッション中にできない
- excel-autoプロジェクトではMCPツールを使わない

**変更**:
- settings.local.jsonからhooksセクションを削除
- MCPツール（mcp__*）の自動許可機能を廃止
- allowリスト方式に簡素化
