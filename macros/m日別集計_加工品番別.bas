...
' 加工品番別集計マクロ（画面ちらつき防止版）
' 「加工1」「加工2」のデータを日付・品番でグループ化して集計し、「通称」列を追加
Sub 日別集計_加工品番別()
    Dim wb As Workbook
    ...
    outputSheetName = "加工品番別"
    outputTableName = "_加工品番別a"
    outputStartCellAddress = "A3"
    
    ' ステータスバー表示
    Application.StatusBar = "加工品番別集計を開始します..."
    ...
    For i = LBound(dataArray, 1) To UBound(dataArray, 1)
        ' 「工程」列の値が「加工1」または「加工2」と完全一致する行を抽出
        If CStr(dataArray(i, colProcess)) = "加工1" Or CStr(dataArray(i, colProcess)) = "加工2" Then
            ' 日付の妥当性チェックと変換
...
NextIteration:
    ...
    If dict.Count = 0 Then
        MsgBox "工程「加工1」「加工2」に該当するデータが集計されませんでした。", vbInformation
    ...
    If Not isNewTable Then
        ...
        On Error GoTo ErrorHandler
    Else
        ' ヘッダー書き込み（新規テーブルの場合のみ）：「通称」列を追加
        outputHeader.Resize(1, 7).Value = Array("日付", "品番", "通称", "実績", "不良", "稼働時間", "段取時間")
    End If
    
    ' 6. 集計結果を配列に変換し、日付・品番でソート
    If dict.Count > 0 Then
        ...
        For r = 1 To dict.Count
            ...
            outputArray(r, 2) = keyParts(1)                 '品番
            outputArray(r, 3) = GetTsusho(CStr(keyParts(1))) '通称
            outputArray(r, 4) = item(0)                     '実績
            outputArray(r, 5) = item(1)                     '不良
            outputArray(r, 6) = item(2)                     '稼働時間
            outputArray(r, 7) = item(3)                     '段取時間
        Next r
        ...
        If Not isNewTable Then
            ...
            tblOutput.Resize outputHeader.Resize(UBound(outputArray, 1) + 1, 7) ' 列数を7に
        End If
    ...
    If isNewTable Then
        ...
        If dict.Count > 0 Then
            Set dataRangeForTable = outputHeader.Resize(dict.Count + 1, 7) ' 列数を7に
        Else
            Set dataRangeForTable = outputHeader.Resize(1, 7) ' ヘッダーのみ、列数を7に
        End If
    ...
    ElseIf dict.Count = 0 Then
        ...
        tblOutput.Resize outputHeader.Resize(1, 7) ' 列数を7に
    End If
    ...
    If Not tblOutput Is Nothing Then
        ...
        If dict.Count > 0 Then
            On Error Resume Next ' 列が存在しない場合のエラーを回避
...
Private Function GetTsusho(hinban As String) As String
    Dim アルヴェルF_品番 As Variant
    Dim アルヴェルR_品番 As Variant
    Dim ノアヴォクF_品番 As Variant
    Dim ノアヴォクR_品番 As Variant
    Dim i As Long

    アルヴェルF_品番 = Array("58020F", "58030F", "58040F", "58050F", "58060F")
    アルヴェルR_品番 = Array("58020R", "58030R", "58040R", "58050R", "58060R")
    ノアヴォクF_品番 = Array("28030F", "28040F")
    ノアヴォクR_品番 = Array("28030R", "28040R")

    For i = LBound(アルヴェルF_品番) To UBound(アルヴェルF_品番)
        If InStr(1, hinban, CStr(アルヴェルF_品番(i)), vbTextCompare) > 0 Then
            GetTsusho = "アルヴェルF"
            Exit Function
        End If
    Next i

    For i = LBound(アルヴェルR_品番) To UBound(アルヴェルR_品番)
        If InStr(1, hinban, CStr(アルヴェルR_品番(i)), vbTextCompare) > 0 Then
            GetTsusho = "アルヴェルR"
            Exit Function
        End If
    Next i

    For i = LBound(ノアヴォクF_品番) To UBound(ノアヴォクF_品番)
        If InStr(1, hinban, CStr(ノアヴォクF_品番(i)), vbTextCompare) > 0 Then
            GetTsusho = "ノアヴォクF"
            Exit Function
        End If
    Next i

    For i = LBound(ノアヴォクR_品番) To UBound(ノアヴォクR_品番)
        If InStr(1, hinban, CStr(ノアヴォクR_品番(i)), vbTextCompare) > 0 Then
            GetTsusho = "ノアヴォクR"
            Exit Function
        End If
    Next i
