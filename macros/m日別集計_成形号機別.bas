...
' 日別集計_成形機別マクロ（全機械対応版）（画面ちらつき防止版）
' 「全工程」シートの「_全工程_成形」テーブルから「成形号機別」シートの「_成形号機別a」テーブルへ集計転記
Sub 日別集計_成形号機別()
    Dim wb As Workbook
    ...
    Dim colDate As Long, colProcess As Long, colKikai As Long
    Dim colJisseki As Long, colDandori As Long, colKadou As Long, colFuryo As Long
    
    Dim currentDate As Date
    Dim currentKikai As String
    ...
    Const PROCESS_FILTER_CRITERIA As String = "成形" ' 「工程」列の固定フィルター条件
    
    ' 基本設定
    Set wb = ThisWorkbook
    sourceSheetName = "全工程_成形"
    sourceTableName = "_全工程_成形"
    outputSheetName = "成形号機別"
    outputTableName = "_成形号機別a"
    outputStartCellAddress = "A3"
    
    ' ステータスバー表示
    Application.StatusBar = "成形号機別集計を開始します..."
    ...
    If colDate = 0 Or colProcess = 0 Or colKikai = 0 Or colJisseki = 0 Or colDandori = 0 Or colKadou = 0 Or colFuryo = 0 Then
        MsgBox "「_全工程_成形」テーブルに必要な列（日付, 工程, 機械, 実績, 段取時間, 稼働時間, 不良）が見つかりません。列名を確認してください。", vbCritical
        GoTo Cleanup
    End If
    ...
    For i = LBound(dataArray, 1) To UBound(dataArray, 1)
        ' 「工程」列に "成形" が含まれる行を抽出 (大文字・小文字を区別しない)
        If InStr(1, CStr(dataArray(i, colProcess)), PROCESS_FILTER_CRITERIA, vbTextCompare) > 0 Then
            ' 日付の妥当性チェックと変換
            ...
            ' 機械の取得
            currentKikai = Trim(CStr(dataArray(i, colKikai)))
            
            ' 複合キーの作成（日付|機械）
            dictKey = Format(currentDate, "yyyy/mm/dd") & "|" & currentKikai
...
NextIteration:
    ...
    If dict.Count = 0 Then
        MsgBox "工程「" & PROCESS_FILTER_CRITERIA & "」に該当するデータが集計されませんでした。", vbInformation
    ...
    Else
        ...
        outputHeader.Resize(1, 6).Value = Array("日付", "機械", "実績", "不良", "稼働時間", "段取時間")
    End If
    
    ' 6. 集計結果を配列に変換し、日付・機械でソート
    If dict.Count > 0 Then
