Attribute VB_Name = "m転記_シートTG作業者別"
Option Explicit

' ==========================================
' TG作業者別からシートへの転記マクロ
' 「_TG作業者別a」テーブルから各作業者シートへデータを転記
' ==========================================
Sub 転記_シートTG作業者別()
    ' ==========================================
    ' 変数宣言
    ...
    ' 進捗表示開始
    Application.StatusBar = "TG作業者別シート転記処理を開始します..."
    ...
    ' ソーステーブル取得
    On Error Resume Next
    Set sourceTable = wsSource.ListObjects("_TG作業者別a")
    If sourceTable Is Nothing Then
        MsgBox "「_TG作業者別a」テーブルが見つかりません。", vbCritical, "テーブルエラー"
        GoTo CleanupAndExit
    End If
    On Error GoTo ErrorHandler
    ...
    If sourceTable.DataBodyRange Is Nothing Then
        MsgBox "「_TG作業者別a」テーブルにデータがありません。", vbInformation, "データなし"
        GoTo CleanupAndExit
    End If
