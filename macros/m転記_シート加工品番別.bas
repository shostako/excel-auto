...
Sub 転記_シート加工品番別()
    ...
    ' 進捗表示開始
    Application.StatusBar = "加工品番別シート転記処理を開始します..."
    ...
    ' ソーステーブル取得
    On Error Resume Next
    Set sourceTable = wsSource.ListObjects("_加工品番別a")
    If sourceTable Is Nothing Then
        MsgBox "「_加工品番別a」テーブルが見つかりません。", vbCritical, "テーブルエラー"
        GoTo CleanupAndExit
    End If
    On Error GoTo ErrorHandler
    ...
    If sourceTable.DataBodyRange Is Nothing Then
        MsgBox "「_加工品番別a」テーブルにデータがありません。", vbInformation, "データなし"
        GoTo CleanupAndExit
    End If
