...
Private Function Create日報塗装複数月結合Query(currentMonth As String, previousMonth As String, _
    ...
    newFormula = newFormula & "    変更された型 = Table.TransformColumnTypes(フィルターされた行1,{" & vbCrLf
    newFormula = newFormula & "        {""日付"", type date}, {""作業者"", type text}, {""開始時間"", type datetime}, {""終了時間"", type datetime}," & vbCrLf
    newFormula = newFormula & "        {""所要時間"", type number}, {""型替"", Int64.Type}, {""停止"", Int64.Type}, {""休憩"", Int64.Type}," & vbCrLf
    newFormula = newFormula & "        {""品番"", type text}, {""L/R"", type text}, {""ロット"", Int64.Type}, {""ショット数"", Int64.Type}," & vbCrLf
    newFormula = newFormula & "        {""リコート"", Int64.Type}, {""廃棄"", Int64.Type}, {""良品数"", Int64.Type}, {""ヒゲ"", type any}," & vbCrLf
    newFormula = newFormula & "        {""ミスト"", type any}, {""ライン"", Int64.Type}, {""ゴミ"", Int64.Type}, {""スケ"", Int64.Type}," & vbCrLf
    newFormula = newFormula & "        {""ピンホール"", Int64.Type}, {""マット"", Int64.Type}, {""その他"", Int64.Type}, {""ゴミ2"", type any}," & vbCrLf
    newFormula = newFormula & "        {""タレ"", Int64.Type}, {""キズ"", Int64.Type}, {""再塗装"", Int64.Type}, {""成形"", Int64.Type}," & vbCrLf
    newFormula = newFormula & "        {""その他2"", Int64.Type}, {""停止内容"", type text}, {""塗装時間"", type number}," & vbCrLf
    newFormula = newFormula & "        {""時間当たり出来高"", type number}, {""不良率"", type number}" & vbCrLf
    newFormula = newFormula & "    })," & vbCrLf
    newFormula = newFormula & "    削除された他の列 = Table.SelectColumns(変更された型,{" & vbCrLf
    ...
    newFormula = newFormula & "    追加_不良数 = Table.AddColumn(削除された他の列, ""不良数"", each" & vbCrLf
    newFormula = newFormula & "        (try Number.From([リコート]) otherwise 0) + (try Number.From([廃棄]) otherwise 0), type number)," & vbCrLf
    newFormula = newFormula & "    削除_リコート廃棄 = Table.RemoveColumns(追加_不良数, {""リコート"",""廃棄""})," & vbCrLf
    newFormula = newFormula & vbCrLf
    newFormula = newFormula & "    // ゴミ + ゴミ2 -> ゴミ" & vbCrLf
    newFormula = newFormula & "    追加_ゴミ_new = Table.AddColumn(削除_リコート廃棄, ""ゴミ_new"", each" & vbCrLf
    newFormula = newFormula & "        (try Number.From([ゴミ]) otherwise 0) + (try Number.From([ゴミ2]) otherwise 0), type number)," & vbCrLf
    ...
    newFormula = newFormula & "    追加_その他_new = Table.AddColumn(名前変更_ゴミ, ""その他O"", each" & vbCrLf
    newFormula = newFormula & "        (try Number.From([その他]) otherwise 0) + (try Number.From([その他2]) otherwise 0), type number)," & vbCrLf
    ...
    newFormula = newFormula & "    置き換えられた値 = Table.ReplaceValue(フィルターされた行,null,0,Replacer.ReplaceValue," & vbCrLf
    newFormula = newFormula & "        {""ショット数"",""ヒゲ"",""ミスト"",""ライン"",""スケ"",""ピンホール"",""マット"",""タレ"",""キズ"",""再塗装"",""成形"",""不良数"",""ゴミ"",""その他O""})," & vbCrLf
    newFormula = newFormula & vbCrLf
    newFormula = newFormula & "    並べ替えられた列 = Table.ReorderColumns(置き換えられた値," & vbCrLf
