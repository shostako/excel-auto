...
' 定数定義
' ============================================================
' バッチ処理: トランザクションを小分けにすることでメモリ効率とロールバック範囲を最適化
' 大量データを一度にコミットすると、エラー時に全データがロールバックされる
' 50件ごとに分割することで、エラー発生時の影響を局所化
Const BATCH_SIZE As Long = 50

' タイムアウト設定: ネットワーク遅延や大量データ処理時のハング防止
' 接続タイムアウト: DB接続確立までの待機時間
' コマンドタイムアウト: SQL実行完了までの待機時間
...
Sub ゾーン別データ転送ADO()
    ...
    ' 画面更新抑制（VBA最適化の基本）
    ' ============================================================
    ' 重要: このマクロはActivate/Selectメソッドを一切使用していない
    ' Activate/Selectが画面ちらつきの真犯人であり、ScreenUpdating=Falseだけでは防げない
    ' オブジェクト参照による直接操作で高速化とちらつき防止を両立
    ...
    ' 重複チェック用キーフィールド設定
    ' ============================================================
    ' 「差戻し」列を含むことで、差戻し状態が異なるレコードを別データとして扱う
    ' 過去の不具合: 差戻し列がキーに含まれていないと、差戻し値が異なるデータが重複判定される
    keyFields = "日付,品番,品番末尾,注番月,ロット,発見,ゾーン,番号,差戻し"

    ' ============================================================
    ' Dictionary オブジェクトによる高速重複チェック
    ' ============================================================
    ' Dictionary: ハッシュテーブルによるO(1)の検索速度（配列のループはO(n)）
    ' 大量データ（数千～数万件）の重複チェックに最適
    ' メモリ使用量: キー文字列のみなので配列より効率的
    ...
    ' ============================================================
    ' ADO接続（Access Database Engine 12.0）
    ' ============================================================
    ' ADODB.Connection: 早期バインディング不要（参照設定なしで動作）
    ' Microsoft.ACE.OLEDB.12.0: Access 2007以降の標準プロバイダー
    ' ネットワークドライブ(Z:)への接続なのでタイムアウト設定が重要
    Set conn = CreateObject("ADODB.Connection")
    Set cmd = CreateObject("ADODB.Command")

    ' 接続文字列: タイムアウト設定でネットワーク遅延に対応
    ...
    ' 既存データの確認（重複転送防止）
    ' ============================================================
    ' パフォーマンス最適化: 日付範囲による絞り込みでSELECT対象を限定
    ' 全件SELECTすると数万～数十万レコードを読み込むため、処理時間が膨大になる
    ' 転送データの日付範囲±7日に限定することで、検索対象を数千件程度に削減
    ...
    ' ============================================================
    ' BeginTrans/CommitTrans/RollbackTransでACID特性を確保
    ' エラー発生時は自動ロールバックにより、中途半端なデータ登録を防止
    ' バッチ処理と組み合わせることで、大量データでも安全に処理
    conn.BeginTrans
    ...
    For i = 1 To rowCount
        ...
        'If True Then                             ' ← 重複チェック無効化（上をコメントアウトしてこちらを有効化）
            ...
            ' バッチコミット（50件ごと）
            ' ============================================================
            ' 利点1: メモリ効率向上（大量データを一度にコミットしない）
            ' 利点2: エラー時の影響範囲限定（全データロールバックを防ぐ）
            ' 利点3: ロック時間短縮（他ユーザーへの影響を最小化）
            ' 利点4: 進捗表示の更新タイミング確保
...
ErrorHandler:
    ' ============================================================
    ' エラーハンドリング（データ整合性の最終防衛線）
    ' ============================================================
    ' errorLocation変数により、エラー発生箇所を特定可能
    ' トランザクションのロールバックで中途半端なデータ登録を防止
    ' バッチコミット採用により、エラー発生時でも一部データは確定済み
    Dim errMsg As String
    errMsg = "エラーが発生しました[" & errorLocation & "]: " & Err.Description & " (エラー番号: " & Err.Number & ")"

    ' トランザクションのロールバック（データ整合性を保証）
    On Error Resume Next
    If Not conn Is Nothing Then
        If conn.state = 1 Then
            If transStarted Then ' トランザクションが開始されている場合のみロールバック
