Attribute VB_Name = "m合計_流出_塗装_期間対応"
Option Explicit

' ========================================
' マクロ名: 合計_流出_塗装_期間対応
' 処理概要: 手直しと廃棄のテーブルを読み込んで流出合計を計算
' 手直しテーブル: シート「塗装」複数テーブル「_手直し塗装_{期間}」
' 廃棄テーブル: シート「塗装」複数テーブル「_廃棄_塗装_{期間}」
' 出力テーブル: シート「塗装」複数テーブル「_流出_塗装_{期間}」
...
Sub 合計_流出_塗装_期間対応()
    ...
    ' ステータスバー初期化
    Application.StatusBar = "流出合計処理（塗装）を開始します..."
    ...
    ' 既存のタイトル行（「流出_塗装」）をV列以降で検索して削除
    Dim searchRange As Range
    Set searchRange = wsTarget.Range("V:XFD")
    If Not searchRange Is Nothing Then
        Dim foundCell As Range
        Set foundCell = searchRange.Find("流出_塗装", LookIn:=xlValues, LookAt:=xlPart)
    ...
    For p = 1 To periodCount
        ...
        handaosiTableName = "_手直し塗装_" & periodName
        haikTableName = "_廃棄_塗装_" & periodName
        ...
        titleText = "流出_塗装_" & periodName & "_" & Format(startDate, "m/d") & "‾" & Format(endDate, "m/d")
        ...
        newTable.Name = "_流出_塗装_" & periodName
...
ErrorHandler:
    ...
    MsgBox "エラーが発生しました" & vbCrLf & _
           ...
           "詳細: " & errDesc, vbCritical, "合計_流出_塗装_期間対応 エラー"
