Attribute VB_Name = "m合計_流出_成形_期間対応"
Option Explicit

' ========================================
' マクロ名: 合計_流出_成形_期間対応
' 処理概要: 手直しと廃棄のテーブルを読み込んで流出合計を計算
' 手直しテーブル: シート「成形」複数テーブル「_手直し成形_{期間}」
' 廃棄テーブル: シート「成形」複数テーブル「_廃棄_成形_{期間}」
' 出力テーブル: シート「成形」複数テーブル「_流出_成形_{期間}」
...
Sub 合計_流出_成形_期間対応()
    ...
    ' ステータスバー初期化
    Application.StatusBar = "流出合計処理（成形）を開始します..."
    ...
    ' 既存のタイトル行（「流出_成形」）をV列以降で検索して削除
    Dim searchRange As Range
    Set searchRange = wsTarget.Range("V:XFD")
    If Not searchRange Is Nothing Then
        Dim foundCell As Range
        Set foundCell = searchRange.Find("流出_成形", LookIn:=xlValues, LookAt:=xlPart)
    ...
    Else
        ' 手直しテーブルがない場合はエラー
        MsgBox "手直しテーブルが見つかりません。", vbExclamation
        GoTo Cleanup
    End If
    ...
    For p = 1 To periodCount
        ...
        handaosiTableName = "_手直し成形_" & periodName
        haikTableName = "_廃棄_成形_" & periodName
        ...
        For i = 1 To baseRowCount
            ...
            For k = 1 To 9
                ...
                If Not handaosiTable Is Nothing And k <= 8 Then
                    If k <= handaosiTable.DataBodyRange.Columns.Count Then
                ...
                ' 廃棄テーブルから値取得
                If Not haikTable Is Nothing Then
                    If k <= haikTable.DataBodyRange.Columns.Count Then
        ...
        titleText = "流出_成形_" & periodName & "_" & Format(startDate, "m/d") & "‾" & Format(endDate, "m/d")
        ...
        newTable.Name = "_流出_成形_" & periodName
...
ErrorHandler:
    ...
    MsgBox "エラーが発生しました" & vbCrLf & _
           ...
           "詳細: " & errDesc, vbCritical, "合計_流出_成形_期間対応 エラー"
