...
' ターゲットテーブル: シート「加工」複数テーブル「_廃棄_加工_{期間}」
...
Sub 転記_廃棄_加工_期間対応()
    ...
    ' ステータスバー初期化
    Application.StatusBar = "廃棄期間対応転記処理（加工）を開始します..."
    ...
    ' 既存のタイトル行（「廃棄_加工」）をK列以降で検索して削除
    Dim searchRange As Range
    Set searchRange = wsTarget.Range("K:XFD")
    If Not searchRange Is Nothing Then
        Dim foundCell As Range
        Set foundCell = searchRange.Find("廃棄_加工", LookIn:=xlValues, LookAt:=xlPart)
    ...
    For p = 1 To periodCount
        ...
        titleText = "廃棄_加工_" & periodName & "_" & Format(startDate, "m/d") & "‾" & Format(endDate, "m/d")
        ...
        newTable.Name = "_廃棄_加工_" & periodName
...
ErrorHandler:
    ...
    MsgBox "エラーが発生しました" & vbCrLf & _
           ...
           "詳細: " & errDesc, vbCritical, "転記_廃棄_加工_期間対応 エラー"
