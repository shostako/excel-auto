...
' ターゲットテーブル: シート「成形」複数テーブル「_廃棄_成形_{期間}」
...
Sub 転記_廃棄_成形_期間対応()
    ...
    ' ステータスバー初期化
    Application.StatusBar = "廃棄期間対応転記処理（成形）を開始します..."
    ...
    ' 既存のタイトル行（「廃棄_成形」）をK列以降で検索して削除
    Dim searchRange As Range
    Set searchRange = wsTarget.Range("K:XFD")
    If Not searchRange Is Nothing Then
        Dim foundCell As Range
        Set foundCell = searchRange.Find("廃棄_成形", LookIn:=xlValues, LookAt:=xlPart)
    ...
    ' 最初の手直しテーブルを動的に探す（"_手直し成形_"で始まる最初のテーブル）
    For Each tbl In wsTarget.ListObjects
    ...
    For p = 1 To periodCount
        ...
        titleText = "廃棄_成形_" & periodName & "_" & Format(startDate, "m/d") & "～" & Format(endDate, "m/d")
        ...
        newTable.Name = "_廃棄_成形_" & periodName
...
ErrorHandler:
    ...
    MsgBox "エラーが発生しました" & vbCrLf & _
           ...
           "詳細: " & errDesc, vbCritical, "転記_廃棄_成形_期間対応 エラー"
