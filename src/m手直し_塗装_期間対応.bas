Attribute VB_Name = "m手直し_塗装_期間対応"
Option Explicit

' ========================================
' マクロ名: 転記_手直し_塗装_期間対応
' 処理概要: 「_集計期間塗装」テーブルの各期間（1‾6行目）に基づく複数期間集計・転記
' ソーステーブル: シート「手直し」テーブル「_手直し」
' 期間テーブル: シート「塗装」テーブル「_集計期間塗装」
' ターゲットテーブル: シート「塗装」複数テーブル「_手直し塗装_{期間}」
' 参照テーブル: シート「塗装」テーブル「_手直し項目塗装」
' 処理方式: 各期間の開始日‾終了日で日付フィルタ後、品番3列による8分類とモード2による項目別集計
' 転記データ: 数量列の合計値（発生=塗装、発見2≠塗装の条件）
...
Sub 転記_手直し_塗装_期間対応()
    ...
    ' ステータスバー初期化
    Application.StatusBar = "期間対応転記処理（塗装）を開始します..."
    ...
    ' 既存の期間別テーブルを削除（"_手直し塗装_"で始まるテーブル全て）
    ...
    ' 既存のタイトル行（「手直し_塗装」）を検索して行削除
    Dim searchRange As Range
    Set searchRange = wsTarget.UsedRange
    If Not searchRange Is Nothing Then
        Dim foundCell As Range
        Set foundCell = searchRange.Find("手直し_塗装", LookIn:=xlValues, LookAt:=xlPart)
    ...
    For p = 1 To periodCount
        ...
        titleText = "手直し_塗装_" & periodName & "_" & Format(startDate, "m/d") & "‾" & Format(endDate, "m/d")
        ...
        newTable.Name = "_手直し塗装_" & periodName
...
NextPeriod:
    ...
    ' 処理完了のステータスバー表示
    Application.StatusBar = "期間対応転記処理（塗装）が完了しました（" & periodCount & "期間）"
...
ErrorHandler:
    ...
    MsgBox "エラーが発生しました" & vbCrLf & _
           ...
           "詳細: " & errDesc, vbCritical, "転記_手直し_塗装_期間対応 エラー"
