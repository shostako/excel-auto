Attribute VB_Name = "m手直し_成形_期間対応"
Option Explicit

' ========================================
' マクロ名: 転記_手直し_成形_期間対応
' 処理概要: 「_集計期間成形」テーブルの各期間に基づく複数期間集計・転記
' ソーステーブル: シート「手直し」テーブル「_手直し」
' 期間テーブル: シート「成形」テーブル「_集計期間成形」
' ターゲットテーブル: シート「成形」複数テーブル「_手直し成形_{期間}」
' 参照テーブル: シート「成形」テーブル「_手直し項目成形」
' 処理方式: 各期間の開始日～終了日で日付フィルタ後、品番3列による8分類とモード2による項目別集計
' 転記データ: 数量列の合計値（発生=成形、発見2≠成形の条件）
' 出力形式: データがある期間のみテーブルを出力（空白期間はスキップ）
' 改善点: 期間テーブルから動的に期間名を取得してテーブル名・タイトルを生成
' ========================================
...
Sub 転記_手直し_成形_期間対応()
    ...
    ' ステータスバー初期化
    Application.StatusBar = "期間対応転記処理（成形）を開始します..."
    ...
    Set wsTarget = ThisWorkbook.Worksheets("成形")

    Dim tblSource As ListObject, tblItems As ListObject, tblPeriod As ListObject
    Set tblSource = wsSource.ListObjects("_手直し")
    Set tblItems = wsTarget.ListObjects("_手直し項目成形")
    Set tblPeriod = wsTarget.ListObjects("_集計期間成形")
    ...
    If periodData Is Nothing Then
        MsgBox "「_集計期間成形」テーブルにデータがありません。", vbExclamation
        GoTo Cleanup
    End If
    ...
    Dim colHizuke As Long, colHinban3 As Long, colHassei As Long, colHakken2 As Long, colMode2 As Long, colSuuryou As Long
    ...
    ' 既存のタイトル行（「手直し_成形」）を検索して行削除
    Dim searchRange As Range
    Set searchRange = wsTarget.UsedRange
    If Not searchRange Is Nothing Then
        Dim foundCell As Range
        Set foundCell = searchRange.Find("手直し_成形", LookIn:=xlValues, LookAt:=xlPart)
    ...
    For p = 1 To periodCount
        ...
        For i = 1 To srcData.Rows.Count
            ...
            If IsDate(srcData.Cells(i, colHizuke).Value) Then
                ...
                If rowDate >= startDate And rowDate <= endDate Then
                    ...
                    hakken2 = CStr(srcData.Cells(i, colHakken2).Value)
        ...
        titleText = "手直し_成形_" & periodName & "_" & Format(startDate, "m/d") & "～" & Format(endDate, "m/d")
        ...
        newTable.Name = "_手直し成形_" & periodName
        newTable.ShowAutoFilter = False ' フィルターボタンを非表示
        newTable.TableStyle = "TableStyleLight21" ' テーブルスタイル設定
...
NextPeriod:
    ...
    ' 処理完了のステータスバー表示
    Application.StatusBar = "期間対応転記処理（成形）が完了しました（" & periodCount & "期間）"
...
ErrorHandler:
    ...
    MsgBox "エラーが発生しました" & vbCrLf & _
           ...
           "詳細: " & errDesc, vbCritical, "転記_手直し_成形_期間対応 エラー"
