...
Sub 期間集計_通称別a()
    ...
    If srcSheet Is Nothing Then
        Application.StatusBar = "通称別直接集計: 「品番別」シートが見つかりません。"
        GoTo CleanUp
    End If
    ...
    ' ステータスバーを更新
    Application.StatusBar = "通称別直接集計: 日付条件を取得中..."
    ...
    If Not tableFound Then
        ...
        If Not IsEmpty(srcSheet.Range("A1").Value) Then
            ...
            If lastRow > 1 Then  ' ヘッダー行とデータがあるときは1行以上
    ...
    If Not tableFound Then
        Application.StatusBar = "通称別直接集計: テーブル「_品番別」が見つかりません。"
        GoTo CleanUp
    End If
    ...
    If hinbanCol = 0 Or dateCol = 0 Then
        Application.StatusBar = "通称別直接集計: 必要な列が見つかりません。"
        GoTo CleanUp
    End If
    ...
    For i = 1 To UBound(srcData, 1)
        ...
        If isInDateRange Then
            ...
            If Not dictGroups.Exists(tsusho) Then
                ...
                dictCounts(tsusho)("サイクル") = 0  ' サイクル用平均計算用
                dictSums(tsusho)("ショット数") = 0
                dictSums(tsusho)("不良数") = 0
                dictSums(tsusho)("打出中") = 0
            ...
            If uchidashiCol > 0 And IsNumeric(srcData(i, uchidashiCol)) Then
                dictSums(tsusho)("打出中") = dictSums(tsusho)("打出中") + CDbl(srcData(i, uchidashiCol))
            End If
            ...
            If gomiCol > 0 And IsNumeric(srcData(i, gomiCol)) Then
                dictSums(tsusho)("ゴミ焼け") = dictSums(tsusho)("ゴミ焼け") + CDbl(srcData(i, gomiCol))
            End If
            ...
            If chocoCol > 0 And IsNumeric(srcData(i, chocoCol)) Then
                dictSums(tsusho)("チョコ停打出中") = dictSums(tsusho)("チョコ停打出中") + CDbl(srcData(i, chocoCol))
            End If
    ...
    destRow = 4
    If useFilter Then
        destSheet.Range("A" & destRow).Value = "通称別不良集計：" & Format(StartDate, "yyyy/mm/dd") & "～" & Format(EndDate, "yyyy/mm/dd")
    Else
        destSheet.Range("A" & destRow).Value = "通称別不良集計：全期間"
    End If
    ...
    destSheet.Range("C" & destRow).Value = "稼動"
    ...
    destSheet.Range("H" & destRow).Value = "打出中"
    ...
    destSheet.Range("O" & destRow).Value = "ゴミ焼け"
    ...
    destSheet.Range("Z" & destRow).Value = "チョコ停打出中"
    ...
    Else
        ...
        For i = 0 To sortIdx - 1
            ...
            destSheet.Cells(destRow, 3).Value = dictSums(key)("稼動")
            ...
            ' 不良項目データを書き込み
            destSheet.Cells(destRow, 8).Value = dictSums(key)("打出中")
            ...
            destSheet.Cells(destRow, 15).Value = dictSums(key)("ゴミ焼け")
            ...
            destSheet.Cells(destRow, 26).Value = dictSums(key)("チョコ停打出中")
...
Function 品番通称判定(hinban As String) As String
    ' 品番から通称を判定する関数
    ' 優先順位: TG > 62-28030 > 62-58050 > 補給品
    
    ' TGチェック（最優先）
    If InStr(hinban, "TG") > 0 Then
        品番通称判定 = "TG"
        Exit Function
    End If
    
    ' 62-28030チェック（Fr/Rr区別）
    If InStr(hinban, "62-28030") > 0 Then
        If InStr(hinban, "Fr") > 0 Or InStr(hinban, "FR") > 0 Or InStr(hinban, "fr") > 0 Then
            品番通称判定 = "62-28030Fr"
        ElseIf InStr(hinban, "Rr") > 0 Or InStr(hinban, "RR") > 0 Or InStr(hinban, "rr") > 0 Then
            品番通称判定 = "62-28030Rr"
        Else
            ' Fr/Rrの記載がない場合はFrとして扱う
            品番通称判定 = "62-28030Fr"
        End If
        Exit Function
    End If
    
    ' 62-58050チェック（Fr/Rr区別）
    If InStr(hinban, "62-58050") > 0 Then
        If InStr(hinban, "Fr") > 0 Or InStr(hinban, "FR") > 0 Or InStr(hinban, "fr") > 0 Then
            品番通称判定 = "62-58050Fr"
        ElseIf InStr(hinban, "Rr") > 0 Or InStr(hinban, "RR") > 0 Or InStr(hinban, "rr") > 0 Then
            品番通称判定 = "62-58050Rr"
