Attribute VB_Name = "m行削除_汎用"

' ========================================
' マクロ名: 選択行削除
' 処理概要: 指定テーブル内で選択行を削除し、同数だけテーブル末尾を追加する
' 引数: tblName - 対象とするテーブル(リストオブジェクト)名
' 備考: シート保護を解除してから実行し、完了後に再保護する
' ========================================
Sub 選択行削除(tblName As String)
    ...
    ' 行削除とテーブル末尾への行追加
    ' ============================================
    Dim firstRow As Long, lastRow As Long, rowsToDelete As Long
    Dim i As Long
    ...
    ' テーブルから選択行を削除（下から上に向かって）
    For i = lastRow To firstRow Step -1
        If (i - tbl.Range.Row + 1) <= tbl.ListRows.Count Then
            tbl.ListRows(i - tbl.Range.Row).Delete
        End If
    Next i

    ' 削除した行数分だけテーブル末尾へ行を追加
