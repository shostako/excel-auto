Attribute VB_Name = "m転記_廃棄_塗装H"
Option Explicit

' ========================================
' マクロ名: 転記_廃棄_塗装H
' 処理概要: 廃棄データを期間別に9分類で集計して塗装Hシートに転記（合計列付き）
'
' 【処理の特徴】
...
'
' 【テーブル構成】
' 項目テーブル : シート「塗装H」、テーブル「_廃棄項目塗装H」
' 期間テーブル : シート「塗装H」、テーブル「_集計期間塗装H」
' ソーステーブル : シート「廃棄」、テーブル「_廃棄」
' 出力テーブル : シート「塗装H」、複数テーブル「_廃棄H_塗装_{期間名}」
'
' 【処理フロー】
' 1. 既存出力テーブルとデータを完全削除
...
'
' 【出力形式】
' - 1行目：不良数（「_廃棄」テーブルの「件数」列を品番2で集計）+ 合計
...
Sub 転記_廃棄_塗装H()
    ...
    If periodCount = 0 Then
        MsgBox "「_集計期間塗装H」に有効な集計期間がありません。処理を中止します。", vbExclamation
        GoTo Cleanup
    End If
    ...
    For idxLO = wsTarget.ListObjects.Count To 1 Step -1
        ...
        If loTemp.Name Like "_廃棄H_塗装_*" Then
            loTemp.Delete
        End If
    Next idxLO
    ...
    For periodIdx = 1 To periodCount
        ...
        titleText = "塗装_廃棄のみ_" & Format(startDate, "m/d") & "‾" & Format(endDate, "m/d")
        ...
        If Not tableRange Is Nothing Then
            ...
            baseName = "_廃棄H_塗装_" & Replace(periodName, " ", "_")
            tryName = baseName
            tryIdx = 1
...
ErrorHandler:
    ...
    MsgBox "エラーが発生しました: " & Err.Description & vbCrLf & _
           "エラー番号: " & Err.Number, vbCritical, "転記_廃棄_塗装H"
End Sub
