Attribute VB_Name = "m転記_手直し_塗装T"
Option Explicit

' ========================================
' マクロ名: 転記_手直し_塗装T
' 処理概要: 手直しデータを期間別に8分類で集計して塗装シートに転記（合計列付き）
'
' 【処理の特徴】
...
'
' 【テーブル構成】
' 期間テーブル : シート「塗装T」、テーブル「_手直し項目塗装T」
' ソーステーブル : シート「手直し」、テーブル「_手直し」
' 項目テーブル : シート「塗装T」、テーブル「_集計期間塗装T」
' 出力テーブル : シート「塗装T」、複数テーブル「_手直しT_塗装_{期間名}」
'
' 【処理フロー】
' 1. 既存出力テーブルとデータを完全削除
...
Sub 転記_手直し_塗装T()
    ...
    Application.StatusBar = "手直し塗装T転記処理を開始します..."
    ...
    Set wsTarget = ThisWorkbook.Worksheets("塗装T")

    Dim tblSource As ListObject, tblItems As ListObject, tblPeriod As ListObject
    On Error Resume Next
    Set tblSource = wsSource.ListObjects("_手直し")
    Set tblItems = wsTarget.ListObjects("_手直し項目塗装T")
    Set tblPeriod = wsTarget.ListObjects("_集計期間塗装T")
    On Error GoTo ErrorHandler
    ...
    If periodCount = 0 Then
        MsgBox "「_集計期間塗装T」に有効な集計期間がありません。処理を中止します。", vbExclamation
        GoTo Cleanup
    End If
    ...
    Dim colHakken2 As Long, colMode2 As Long, colSuuryou As Long
    ...
    For idxLO = wsTarget.ListObjects.Count To 1 Step -1
        ...
        If loTemp.Name Like "_手直しT_塗装_*" Then
            loTemp.Delete
        End If
    Next idxLO
    ...
    For periodIdx = 1 To periodCount
        ...
        titleText = "塗装_手直しのみ_" & Format(startDate, "m/d") & "‾" & Format(endDate, "m/d")
        ...
        If Not tableRange Is Nothing Then
            ...
            baseName = "_手直しT_塗装_" & Replace(periodName, " ", "_")
            tryName = baseName
            tryIdx = 1
...
ErrorHandler:
    ...
    MsgBox "エラーが発生しました: " & Err.Description & vbCrLf & _
           "エラー番号: " & Err.Number, vbCritical, "転記_手直し_塗装T"
End Sub
