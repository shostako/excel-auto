Attribute VB_Name = "m転記_手直し_成形T"
Option Explicit

' ========================================
' マクロ名: 転記_手直し_成形T
' 処理概要: 手直しデータを期間別に8分類で集計して成形シートに転記（合計列付き）
'
' 【処理の特徴】
' 1. 空白期間対応：データがなくてもテーブル構造（ヘッダー・不良数行）を出力
' 2. 動的期間対応：集計期間テーブルの行数が変わっても自動的に対応（増減どちらもOK）
' 3. 高速化：配列処理による大量データの高速集計
' 4. ワースト順機能：項目テーブルの「ワースト」設定に応じて動的に出力順序を変更
' 5. 合計列追加：8品番の右に合計列を追加して各行の合計値を自動計算
'
' 【テーブル構成】
' 期間テーブル : シート「成形T」、テーブル「_手直し項目成形T」
' ソーステーブル : シート「手直し」、テーブル「_手直し」
' 項目テーブル : シート「成形T」、テーブル「_集計期間成形T」
' 出力テーブル : シート「成形T」、複数テーブル「_手直しT_成形_{期間名}」
'
' 【処理フロー】
' 1. 既存出力テーブルとデータを完全削除
...
' 5. データがない期間もテーブル構造を出力（不良数は0表示）
' 6. 各行に8品番の合計列を追加
'
' 【出力形式】
' - 1行目：不良数（「_手直し」テーブルの「数量」列を品番3で集計）+ 合計
' - 2行目以降：ワースト順で項目別集計 + 合計
'   - 「全項目」設定：0でない項目を降順で全て出力
'   - 数値N設定：上位N件 + 「その他」行（N+1行、ただし0でない項目数<=Nなら「その他」なし）
'
' 【集計条件】
' - 「_手直し」テーブル：発生=成形 AND 発見2≠成形
' - 品番照合：完全一致
' ========================================
...
Sub 転記_手直し_成形T()
    ...
    Application.StatusBar = "手直し成形T転記処理を開始します..."
    ...
    Set wsTarget = ThisWorkbook.Worksheets("成形T")

    Dim tblSource As ListObject, tblItems As ListObject, tblPeriod As ListObject
    On Error Resume Next
    Set tblSource = wsSource.ListObjects("_手直し")
    Set tblItems = wsTarget.ListObjects("_手直し項目成形T")
    Set tblPeriod = wsTarget.ListObjects("_集計期間成形T")
    On Error GoTo ErrorHandler
    ...
    If Not tblPeriod Is Nothing Then
        If Not tblPeriod.DataBodyRange Is Nothing Then
            ...
            If totalRows > 0 Then
                ReDim periodInfo(1 To totalRows, 1 To 3)
                Dim p As Long
                Dim pIdx As Long
                pIdx = 0
                ...
                For p = 1 To totalRows
                    ...
                    ' 空行スキップ: IsEmpty + IsDate + CDbl <> 0 の3段階検証
    ...
    If periodCount = 0 Then
        MsgBox "「_集計期間成形T」に有効な集計期間がありません。処理を中止します。", vbExclamation
        GoTo Cleanup
    End If
    ...
    Dim colHizuke As Long, colHinban3 As Long, colHassei As Long, colHakken2 As Long
    ...
    colHakken2 = tblSource.ListColumns("発見2").Index
    ...
    For idxLO = wsTarget.ListObjects.Count To 1 Step -1
        ...
        If loTemp.Name Like "_手直しT_成形_*" Then
            loTemp.Delete
        End If
    Next idxLO
    ...
    For periodIdx = 1 To periodCount
        ...
        For r = 1 To UBound(srcArr, 1)
            ...
            If IsDate(srcDate) Then
                ...
                If srcDt >= startDate And srcDt <= endDate Then
                    ...
                    hakken2 = Trim(CStr(srcArr(r, colHakken2)))
        ...
        titleText = "成形_手直しのみ_" & Format(startDate, "m/d") & "‾" & Format(endDate, "m/d")
        ...
        If Not tableRange Is Nothing Then
            ...
            baseName = "_手直しT_成形_" & Replace(periodName, " ", "_")
            tryName = baseName
            tryIdx = 1
    ...
    If printRangeStart > 0 And printRangeEnd > 0 Then
        ...
        printLastCol = UBound(allGroups) + 3  ' 項目列 + 8品番 + 合計列
...
ErrorHandler:
    ...
    MsgBox "エラーが発生しました: " & Err.Description & vbCrLf & _
           "エラー番号: " & Err.Number, vbCritical, "転記_手直し_成形T"
    Exit Sub
