Attribute VB_Name = "m転記_日報_塗装NW"
Option Explicit

' ========================================
' マクロ名: 転記_日報_塗装NW
' 処理概要: 日報データを期間別に9分類で集計して塗装シートに転記（ワースト順対応版）
'
' 【処理の特徴】
...
' 5. その他O特殊処理：「その他O」は常に「その他」行に集計、ワースト順対象外
'
' 【テーブル構成】
' 期間テーブル : シート「塗装NW」、テーブル「_集計期間日報塗装W」
' ソーステーブル : シート「日報塗装」、テーブル「_日報塗装」
' 項目テーブル : シート「塗装NW」、テーブル「_日報項目塗装W」
' 出力テーブル : シート「塗装NW」、複数テーブル「_日報W_塗装_{期間名}」
'
' 【処理フロー】
' 1. 既存出力テーブルとデータを完全削除
' 2. ワースト設定（全項目 or 数値N）を読み込み
' 3. 各期間ごとに日付フィルター + 品番による9分類集計
' 4. 集計結果を降順ソートしてワースト順出力（「その他O」は別枠）
...
' - 2行目：不良数（各項目の合計値）
' - 3行目以降：ワースト順で項目別集計
'   - 「全項目」設定：0でない項目を降順で全て出力、「その他O」は「その他」に改名して最終行
'   - 数値N設定：上位N件 + 「その他」行（「その他O」含む）
' - 最終列：合計
'
' 【集計方法（塗装特有）】
' 品番2列の値で9分類を判定：
' - 8つの固定品番（58050FrLH/RH, 58050RrLH/RH, 28050FrLH/RH, 28050RrLH/RH）→ そのまま該当グループへ
' - それ以外は全て「補給品」に振り分け
' - 倍率処理なし（成形のような「SET×2」はない）
' ========================================
...
Sub 転記_日報_塗装NW()
    ...
    Application.StatusBar = "日報塗装W転記処理を開始します..."
    ...
    Set wsTarget = ThisWorkbook.Worksheets("塗装NW")
    ...
    Set tblItems = wsTarget.ListObjects("_日報項目塗装W")
    Set tblPeriod = wsTarget.ListObjects("_集計期間日報塗装W")
    On Error GoTo ErrorHandler
    ...
    ' 集計期間が1つもなければ処理中止
    If periodCount = 0 Then
        MsgBox "「_集計期間日報塗装W」に有効な集計期間がありません。処理を中止します。", vbExclamation
        GoTo Cleanup
    End If
    ...
    targetColumns = Array("ショット数", "不良数", "リコート", "廃棄", "ヒゲ", "ミスト", "ライン", "スケ", "ピンホール", _
                          "マット", "タレ", "キズ", "再塗装", "成形", "ゴミ", "その他O")

    ' ワースト順集計対象項目（「その他O」以外の11項目）
    ' 理由：「その他O」は常に「その他」行に集計するため、ワースト順対象外
    Dim worstTargetItems As Variant
    worstTargetItems = Array("ヒゲ", "ミスト", "ライン", "スケ", "ピンホール", _
                             "マット", "タレ", "キズ", "再塗装", "成形", "ゴミ")
    ...
    For idxLO = wsTarget.ListObjects.Count To 1 Step -1
        ...
        If loTemp.Name Like "_日報W_塗装_*" Then
    ...
    For periodIdx = 1 To periodCount
        ...
        Set aggSonotaO = CreateObject("Scripting.Dictionary")
        ...
        For Each grp In allGroups
            ...
            aggHaiki(CStr(grp)) = 0
            aggSonotaO(CStr(grp)) = 0

            ' 項目別集計用のネストDictionary（11項目、「その他O」除く）
        ...
        For r = 1 To totalRows
            ...
            If IsDate(cellDate) Then
                ...
                If dt >= startDate And dt <= endDate Then
                    ...
                    Else
                        targetGroup = "補給品"  ' 8つの固定品番に該当しない場合
                    End If
                    ...
                    For Each keyName In colIndexes.Keys
                        ...
                        If colIdxSrc >= 1 And colIdxSrc <= UBound(srcArr, 2) Then
                            ...
                            If IsNumeric(colValue) Then
                                ...
                                ElseIf CStr(keyName) = "その他O" Then
                                    ' その他Oは別枠で管理
                                    aggSonotaO(targetGroup) = aggSonotaO(targetGroup) + numValue
                                Else
                                    ' 11項目のいずれか
        ...
        titleText = "日報W_塗装_" & periodName & "_" & Format(startDate, "m/d") & "‾" & Format(endDate, "m/d")
        ...
        ' ============================================
        ' 5行目以降：ワースト順で項目別集計
        ' 理由：全グループの合計値でワースト順を決定する（「その他O」除く11項目）
        ' ============================================

        ' 全グループの項目別合計を計算（11項目のみ）
        ...
        ' 理由：「その他」行の出力有無判定に使用
        ' ============================================
        Dim totalSonotaO As Double
        totalSonotaO = 0
        For Each grp In allGroups
            totalSonotaO = totalSonotaO + CDbl(aggSonotaO(CStr(grp)))
        Next grp
        ...
        If isAllItems Then
            ...
            ' 「その他O」が0でなければ「その他」行を出力
            If totalSonotaO <> 0 Then
                hasSonotaRow = True
            End If
        Else
            ' 数値Nモード：上位N件 + その他
        ...
        If hasSonotaRow Then
            ...
            For Each grp In allGroups
                ...
                ' 「その他O」を加算
                sonotaSum = sonotaSum + CDbl(aggSonotaO(CStr(grp)))

                ' 数値Nモード（多い）の場合：上位N件以外を加算
                If Not isAllItems And nonZeroCount > worstNum Then
                    ' 上位N件に含まれない項目を特定
                    Dim k As Long
        ...
        If Not tableRange Is Nothing Then
            ...
            baseName = "_日報W_塗装_" & Replace(periodName, " ", "_")
            tryName = baseName
            tryIdx = 1
...
ErrorHandler:
    ...
    MsgBox "エラーが発生しました: " & Err.Description & vbCrLf & _
           "エラー番号: " & Err.Number, vbCritical, "転記_日報_塗装W"
End Sub
