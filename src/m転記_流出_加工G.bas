...
' ========================================
' マクロ名: 転記_流出_加工G
' 処理概要: 手直しと廃棄データを統合し期間別に9分類で集計して加工Gシートに転記
'
' 【処理の特徴】
...
'
' 【テーブル構成】
' 期間テーブル : シート「加工G」、テーブル「_集計期間加工G」
' 項目テーブル : シート「加工G」、テーブル「_流出項目加工G」
' ソーステーブル : シート「手直し」、テーブル「_手直し」
'                  シート「廃棄」、テーブル「_廃棄」
'                  シート「ロット数量」、テーブル「_ロット数量」
' 出力テーブル : シート「加工G」、複数テーブル「_流出G_加工_{期間名}」
'
' 【処理フロー】
' 1. 既存出力テーブルとデータを完全削除
...
'    b) 手直しテーブルから不良数と項目別集計（発生=加工）
'    c) 廃棄テーブルから不良数と項目別集計（工程=加工）
'    d) 手直しと廃棄の同一項目名を合算
'    e) 全グループ合計で降順ソート
' 4. データがある期間のみテーブル出力（空白期間はスキップ）
'
' 【出力形式】
' - 1行目：ショット数（「_ロット数量」テーブルで工程=加工、品番2で9分類照合）
' - 2行目：不良数（手直しの数量 + 廃棄の件数）
' - 3行目以降：ワースト順で項目別集計（手直しのモード2 + 廃棄の不良内容を統合）
'   - 「全項目」設定：0でない項目を降順で全て出力
'   - 数値N設定：上位N件 + 「その他」行（N+1行、ただし0でない項目数<=Nなら「その他」なし）
'
' 【集計条件】
' - 手直し：発生=加工、品番3（8分類→9分類にマッピング）
' - 廃棄：工程=加工、品番2（9分類）
' - 品番照合：完全一致
' ========================================
...
Sub 転記_流出_加工G()
    ...
    Dim colHandaosiHizuke As Long, colHinban3 As Long, colHassei As Long
    ...
    For periodIdx = 1 To periodCount
        ...
        colOffset = 2
        For Each grp In allGroups
            wsTarget.Cells(rowIdx, colOffset).Value = aggShot(CStr(grp))
            colOffset = colOffset + 1
        Next grp
        rowIdx = rowIdx + 1
        ...
        colOffset = 2
        For Each grp In allGroups
            wsTarget.Cells(rowIdx, colOffset).Value = aggFuryo(CStr(grp))
            colOffset = colOffset + 1
        Next grp
        rowIdx = rowIdx + 1
...
NextPeriod:
    ...
    If printRangeStart > 0 And printRangeEnd > 0 Then
        ...
        printLastCol = UBound(allGroups) + 2
