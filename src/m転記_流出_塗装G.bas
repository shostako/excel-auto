Attribute VB_Name = "m転記_流出_塗装G"
Option Explicit

' ========================================
' マクロ名: 転記_流出_塗装G
' 処理概要: 手直しと廃棄データを統合し期間別に9分類で集計して塗装Gシートに転記（合計列付き）
' 集計条件: 手直し(発生=塗装 AND 発見2≠塗装、品番3→品番2マッピング) + 廃棄(工程=塗装、品番2)
' ========================================
...
Sub 転記_流出_塗装G()
    ...
    Application.StatusBar = "流出塗装G転記処理を開始します..."
    ...
    Set wsTarget = ThisWorkbook.Worksheets("塗装G")
    ...
    Set tblItems = wsTarget.ListObjects("_流出項目塗装G")
    Set tblPeriod = wsTarget.ListObjects("_集計期間塗装G")
    On Error GoTo ErrorHandler
    ...
    If periodCount = 0 Then
        MsgBox "「_集計期間塗装G」に有効な集計期間がありません。処理を中止します。", vbExclamation
        GoTo Cleanup
    End If
    ...
    Dim colHdHakken2 As Long, colHdMode2 As Long, colHdSuuryou As Long
    If Not handaosiData Is Nothing Then
        ...
        colHdHakken2 = tblHandaosi.ListColumns("発見2").Index
    ...
    For idxLO = wsTarget.ListObjects.Count To 1 Step -1
        ...
        If loTemp.Name Like "_流出G_塗装_*" Then
            loTemp.Delete
        End If
    Next idxLO
    ...
    Dim handaosiArr As Variant, haikiArr As Variant
    If Not handaosiData Is Nothing Then
    ...
    For periodIdx = 1 To periodCount
        ...
        ' 手直しテーブルから不良数と項目別集計（発生=塗装 AND 発見2≠塗装、品番3→品番2マッピング）
        If Not handaosiData Is Nothing Then
            Dim r As Long
            For r = 1 To UBound(handaosiArr, 1)
                ...
                If IsDate(hdDate) Then
                    ...
                    If hdDt >= startDate And hdDt <= endDate Then
                        Dim hdHassei As String, hdHakken2 As String
                        hdHassei = Trim(CStr(handaosiArr(r, colHdHassei)))
                        hdHakken2 = Trim(CStr(handaosiArr(r, colHdHakken2)))

                        If hdHassei = "塗装" And hdHakken2 <> "塗装" Then
        ...
        titleText = "塗装_流出手直し（廃棄込）" & Format(startDate, "m/d") & "‾" & Format(endDate, "m/d")
        ...
        If Not tableRange Is Nothing Then
            ...
            baseName = "_流出G_塗装_" & Replace(periodName, " ", "_")
            tryName = baseName
            tryIdx = 1
...
ErrorHandler:
    ...
    MsgBox "エラーが発生しました: " & Err.Description & vbCrLf & _
           "エラー番号: " & Err.Number, vbCritical, "転記_流出_塗装G"
End Sub
