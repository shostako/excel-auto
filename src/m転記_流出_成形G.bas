...
' 3. 各期間ごとに：
'    a) ロット数量からショット数集計（工程=加工）
'    b) 手直しテーブルから不良数と項目別集計（発生=成形 AND 発見2≠成形）
'    c) 廃棄テーブルから不良数と項目別集計（工程=成形）
'    d) 手直しと廃棄の同一項目名を合算
'    e) 全グループ合計で降順ソート
...
' - 2行目：不良数（手直しの数量 + 廃棄の件数）+ 合計
' - 3行目以降：ワースト順で項目別集計（手直しのモード2 + 廃棄の不良内容を統合）+ 合計
...
Sub 転記_流出_成形G()
    ...
    For periodIdx = 1 To periodCount
        ...
        titleText = "流出G_成形_" & periodName & "_" & Format(startDate, "m/d") & "‾" & Format(endDate, "m/d")
