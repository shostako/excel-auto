Attribute VB_Name = "m転記_統合_成形P"
Option Explicit

' ========================================
' マクロ名: 転記_統合_成形P
' 処理概要: 成形G/T/Hの出力テーブルを成形Pシートに統合コピーし、印刷用レイアウトを作成
'
' 【処理の特徴】
' 1. 従来マクロ流用：成形G/T/Hの出力テーブルをそのままコピー
' 2. 統合表示：G→T→Hの順に縦並びで配置
' 3. 期間別ページ：各期間で改ページを挿入
' 4. 印刷対応：全体を印刷範囲に設定
'
' 【テーブル構成】
' 期間テーブル : シート「成形P」、テーブル「_集計期間成形P」
' コピー元テーブル :
'   - シート「成形G」、テーブル「_流出G_成形_{期間名}」
'   - シート「成形T」、テーブル「_手直しT_成形_{期間名}」
'   - シート「成形H」、テーブル「_廃棄H_成形_{期間名}」
'
' 【処理フロー】
' 1. 成形Pシートの既存データ・改ページをクリア
' 2. 期間テーブルから期間名リストを取得
...
Sub 転記_統合_成形P()
    ...
    Application.StatusBar = "成形P統合転記処理を開始します..."
    ...
    Set wsTarget = ThisWorkbook.Worksheets("成形P")
    Set wsG = ThisWorkbook.Worksheets("成形G")
    Set wsT = ThisWorkbook.Worksheets("成形T")
    Set wsH = ThisWorkbook.Worksheets("成形H")

    Dim tblPeriod As ListObject
    On Error Resume Next
    Set tblPeriod = wsTarget.ListObjects("_集計期間成形P")
    On Error GoTo ErrorHandler

    If tblPeriod Is Nothing Then
        MsgBox "シート「成形P」にテーブル「_集計期間成形P」が見つかりません。", vbCritical
        GoTo Cleanup
    End If
    ...
    On Error Resume Next
    Set tblItems = wsTarget.ListObjects("_流出項目成形P")
    On Error GoTo ErrorHandler
    ...
    For periodIdx = 1 To validPeriodCount
        ...
        ' テーブル名を構築
        tblNameG = "_流出G_成形_" & Replace(periodNames(periodIdx), " ", "_")
        tblNameT = "_手直しT_成形_" & Replace(periodNames(periodIdx), " ", "_")
        tblNameH = "_廃棄H_成形_" & Replace(periodNames(periodIdx), " ", "_")
    ...
    Application.StatusBar = "成形P統合転記処理が完了しました"
    Debug.Print "成形P統合転記完了: " & validPeriodCount & "期間処理"
