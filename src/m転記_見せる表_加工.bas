...
'   - シート「加工NW」テーブル「_日報W_加工_期間1」（ショット数取得用）
...
' - ショット数は加工NWから取得（加工は最終工程のため不良数はG系から）
' ========================================
...
Sub 転記_見せる表_加工()
    ...
    Set 期間TblNW = Sheets("加工NW").ListObjects("_集計期間日報加工W")
    ...
    If Not 期間1見つかったNW Then
        MsgBox "エラー: 加工NWシートに「期間1」が見つかりません。", vbCritical
        GoTo ErrorHandler
    End If

    ' 期間一致チェック
    If 開始日NW <> 開始日 Or 終了日NW <> 終了日 Then
        MsgBox "エラー: 加工NWと加工Gの期間1が一致しません。" & vbCrLf & _
               "加工NW: " & Format(開始日NW, "yyyy/mm/dd") & " ‾ " & Format(終了日NW, "yyyy/mm/dd") & vbCrLf & _
               "加工G: " & Format(開始日, "yyyy/mm/dd") & " ‾ " & Format(終了日, "yyyy/mm/dd"), vbCritical
        GoTo ErrorHandler
    End If
    ...
    ' 工程内不良用（加工NWソース）- ショット数取得用
    Dim 工程内Tbl As ListObject
    Set 工程内Tbl = Sheets("加工NW").ListObjects("_日報W_加工_期間1")

    If 工程内Tbl Is Nothing Then
        MsgBox "エラー: シート「加工NW」にテーブル「_日報W_加工_期間1」が見つかりません。", vbCritical
        GoTo ErrorHandler
    End If

    If 工程内Tbl.DataBodyRange Is Nothing Then
        MsgBox "エラー: テーブル「_日報W_加工_期間1」にデータがありません。", vbCritical
        GoTo ErrorHandler
    End If
    ...
    ' --------------------------------------------
    ' 6-3. ショット数を加工NWから転記（7行目）
    ' --------------------------------------------
    ' 理由: 加工は最終工程のためショット数は日報から取得
    ' ポイント: 不良数はG系から取得するが、ショット数はNWから

    Application.StatusBar = "ショット数を転記中..."
    ...
    For i = 1 To 工程内Tbl.ListRows.Count
        ...
        If Trim(itemNW) = "ショット数" Then
            Debug.Print "→ ショット数を7行目に転記（加工NWから）"
    ...
    For i = 1 To 流出Tbl.ListRows.Count
        ...
        If Trim(item) = "ショット数" Then
            ' ショット数は加工NWから転記済みなのでスキップ
            Debug.Print "→ ショット数はスキップ（加工NWから転記済み）"
