        ...
        For Each key In 品番マスタ.Keys
            ...
            For wdIdx = 1 To workDays.Count
                ...
                If 割り当て > 0 Then
                    ...
                    ' 型番割り当て累積（セット品のみ）
                    Dim セット値 As String, 型番値 As String
                    セット値 = CStr(品番マスタ(品番)("セット"))
                    If セット値 = "SET" Then
                        型番値 = CStr(品番マスタ(品番)("型番"))
                        If 型番値 <> "" Then
                            Dim 型番日キー値 As String
                            型番日キー値 = 型番値 & "_" & 対象稼働日
                            If 当日型番割り当て.Exists(型番日キー値) Then
                                当日型番割り当て(型番日キー値) = CLng(当日型番割り当て(型番日キー値)) + 割り当て
                            Else
                                当日型番割り当て(型番日キー値) = 割り当て
                            End If
                            Debug.Print "  → 型番[" & 型番値 & "] 累積: " & 当日型番割り当て(型番日キー値) & "個"
                        End If
                    End If
            ...
            If CLng(月間残数(品番)) > 0 Then
                ...
                For wdIdx = workDays.Count To 1 Step -1
                    ...
                    If 割り当て > 0 Then
                        ...
                        ' 型番割り当て累積（セット品のみ）
                        Dim 残数セット値 As String, 残数型番値 As String
                        残数セット値 = CStr(品番マスタ(品番)("セット"))
                        If 残数セット値 = "SET" Then
                            残数型番値 = CStr(品番マスタ(品番)("型番"))
                            If 残数型番値 <> "" Then
                                Dim 残数型番日キー値 As String
                                残数型番日キー値 = 残数型番値 & "_" & 稼働日
                                If 当日型番割り当て.Exists(残数型番日キー値) Then
                                    当日型番割り当て(残数型番日キー値) = CLng(当日型番割り当て(残数型番日キー値)) + 割り当て
                                Else
                                    当日型番割り当て(残数型番日キー値) = 割り当て
                                End If
                            End If
                        End If
