Attribute VB_Name = "m転記_負荷均し_完全版"
Option Explicit
...
Sub 転記_負荷均し()
    ...
    Application.StatusBar = "負荷均し処理を開始します（完全版）..."
...
NextRow:
    ...
    For 優先度 = 1 To 3
        ...
        Call 系列別均し処理("モール", "アルヴェル系", 優先度, _
            ...
            workDays, 日次目標, 誤差許容率, maxDay, 開始列, 成形品番列, arr展開)
...
Private Function 当日号口単品配置済み( _
    ...
    For Each key In 転記データ.Keys
        ...
        ' 同じ日かつ異なる品番で号口単品があるかチェック
        If 日 = 稼働日 And 品番 <> 除外品番 Then
