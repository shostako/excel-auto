Attribute VB_Name = "m転記_集計表_塗装品番別_改6"
Option Explicit

' ========================================
' マクロ名: 転記_集計表_塗装品番別
' 処理概要: 塗装品番別テーブルデータを集計表シートの塗装別欄に転記
' ソーステーブル: シート「塗装品番別」テーブル「_塗装品番別b」
' ターゲットシート: シート「集計表」
...
Sub 転記_集計表_塗装品番別()
    ...
    ' ステータスバー初期化
    Application.StatusBar = "塗装品番別 集計表転記を開始..."
    ...
    If wsSource Is Nothing Then
        Application.StatusBar = "塗装品番別シートが見つかりません"
        GoTo Cleanup
    End If
    On Error GoTo ErrorHandler
    ...
    If sourceTable Is Nothing Then
        Application.StatusBar = "テーブル_塗装品番別bが見つかりません"
        GoTo Cleanup
    End If
    On Error GoTo ErrorHandler
...
NextIteration:
            ...
            If processedCount Mod 5 = 0 Or processedCount = totalProcesses Then
                Application.StatusBar = "塗装集計表転記中... " & Format(processedCount / totalProcesses, "0%") & _
    ...
    dandoriTransfers = Array( _
        Array("合計日平均段取時間", 20, 20), _
        Array("合計日段取時間", 21, 20), _
        Array("合計日段取回数", 22, 20), _
        Array("合計累計段取時間", 23, 20), _
        Array("合計累計段取回数", 24, 20), _
        Array("合計平均段取時間", 25, 20) _
    )
    ...
    ' 処理完了表示
    Application.StatusBar = "塗装集計表転記完了: " & processedCount & "件のデータを転記"
...
ErrorHandler:
    ...
    ' エラーメッセージの表示
    MsgBox "塗装品番別 集計表転記でエラー発生" & vbCrLf & _
