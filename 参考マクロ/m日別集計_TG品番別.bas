...
' TG品番別集計マクロ（画面ちらつき防止版）
' 「加工3」のデータを日付・品番でグループ化して集計
Sub 日別集計_TG品番別()
    Dim wb As Workbook
    ...
    outputSheetName = "TG品番別"
    outputTableName = "_TG品番別a"
    outputStartCellAddress = "A3"
    
    ' ステータスバー表示
    Application.StatusBar = "TG品番別集計を開始します..."
...
NextIteration:
    ...
    Else
        ...
        outputHeader.Resize(1, 6).Value = Array("日付", "品番", "実績", "不良", "稼働時間", "段取時間")
    End If
