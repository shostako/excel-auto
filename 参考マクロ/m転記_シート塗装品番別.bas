...
Sub 転記_シート塗装品番別()
    ...
    Application.StatusBar = "塗装品番別 転記処理を開始..."
    
    On Error GoTo ErrorHandler
    
    ' テーブル取得
    Dim ws As Worksheet
    Set ws = ThisWorkbook.Worksheets("塗装品番別")
    
    Dim srcTable As ListObject, tgtTable As ListObject
    Set srcTable = ws.ListObjects("_塗装品番別a")
    Set tgtTable = ws.ListObjects("_塗装品番別b")
    ...
    For Each key In dateNicknameTotals.Keys
        ...
        If processedKeys Mod 10 = 0 Or processedKeys = totalKeys Then
            Application.StatusBar = "塗装品番別 転記処理中... " & Format(processedKeys / totalKeys, "0%") & _
    ...
    ' 完了時のステータスバー表示
    Application.StatusBar = "塗装品番別転記完了: " & transferred & "件の品番別データ、" & _
...
ErrorHandler:
    ' エラー時だけメッセージボックス
    MsgBox "塗装品番別 転記処理でエラー発生" & vbCrLf & vbCrLf & _
